...
from config import CONNECT_INTERVAL, MAX_CONNECTIONS, MIN_CONNECTIONS, RESPONSE_TIMEOUT, PING_INTERVAL, UPDATE_INTERVAL
...
class Client:
    ...
    def connect_to_nodes(self) -> None:
        ...
        """
        while True:
            print("server_connections " + str(self.client_connections))
            print("known_nodes" + str(self.node.known_nodes))
            if len(self.client_connections) >= MAX_CONNECTIONS:
                pass
            elif len(self.client_connections) < MIN_CONNECTIONS:
                ...
                for n in known_nodes:
                    ...
                    for j in self.client_connections:
                        try:
                            if n != j.getpeername():
                                current_node = n
                                break
                            ...
                            if len(addresses) < 100:
                                for addr in addresses:
                                    if addr not in self.node.known_nodes:
                                        if Utils.check_node_working(self.node.NODE_IP, self.node.NODE_PORT, addr):
